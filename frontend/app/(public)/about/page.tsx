...
async function getCMSPage(slug: string) {
  try {
    const res = await fetch(`${API_URL}/cms/slug/${slug}`, { next: { revalidate: 60 } });
...
export async function generateMetadata(): Promise<Metadata> {
  return {
    ...
    description: 'Learn about Kali Material Handling Solutions — our history, leadership, manufacturing capabilities, and global reach.',
  };
}
...
export default async function AboutPage() {
  ...
  const hero = cms?.hero ?? {
    badge: 'About Kali MHS', title: 'Built on a Legacy of', titleHighlight: 'Engineering Excellence',
    subtitle: 'Since 1972, Kali Material Handling Solutions has been at the forefront of the industry.',
  ...
  const stats: { value: string; label: string }[] = cms?.stats ?? [
    { value: '50+', label: 'Years of Excellence' }, { value: '500+', label: 'Projects Delivered' },
    { value: '30+', label: 'Countries Served' }, { value: '98%', label: 'Client Satisfaction' },
  ];
  const cta = cms?.cta ?? {
    title: 'Ready to Partner with Us?', subtitle: '',
    primaryButton: { label: 'Request a Quote', href: '/rfq' },
